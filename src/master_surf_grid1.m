...
%     KK = 100*KK;
    ...
    TLmixingFlag = 1; %Flag to enable Todd-Longstaff mixing parameter
    ...
    src_total = 0;
    sumUU = 0;
    % Define the folder name within the current working directory
output_folder = fullfile(pwd, 'Run1');
...
%%%%%%%%%% Get Visualization of saturation/concentration profiles ---------
    ...
    fig = figure('Visible', 'off');  % Set figure visibility to off
...
% 
          %if rem(tcal,1)==0
        %       saveas(gcf, sprintf('./Run%d/ST%dK%dx%d.jpeg', counter, tcal, size(KK,1), size(KK,2)));
 ...
 % Create the filename with the path to the output folder
    filename = fullfile(output_folder, sprintf('viscosity_%d.jpeg', t));
   
    % Save the plot as a JPEG file in the specified folder
    saveas(gcf, filename);
...
csvwrite('lambdaTcal1.csv',lambdaTcal)
csvwrite('miuaTcal1.csv',miuaTcal)
csvwrite('COC1.csv',COC)  
